...
//
// snki.cc - a Snek interpreter.
//
// Usage: ./snki [--test] [--tokens] [--pprint] <Snek source file name>
//
...
//
//    --dump - spit out the abstract syntax tree of the parsed source code.
//
// The code is heavily reliant upon:
//
//   snek-ast.{cc,hh} - defines the AST for Snek programs
//   snek-lex.{cc,hh} - converts the source into a token stream
//   snek-parse.{cc,hh} - parse a Snek token stream
//
...
int main(int argc, char** argv) {
    ...
    bool  dump_ast  = check_flag(argc,argv,"--dump");
    ...
    if (filename) {
        ...
        try {
            ...
            if (!tks.at_EOF()) {
                ...
                //
                throw SnekError { tks.locate(), "Error: extra unparsed characters.\n" };
            }
