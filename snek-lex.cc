...
// snek-lex.cc
//
// An implementation of a lexical analysis tool for the Snek language
...
//
// A Tokenizer instance reads the text of Snek source code, consumes it
...
bool is_string(std::string s);   // Is the token string a string literal?
bool is_operator(std::string s); // Is the token string an operator?

// * * * * *
//
// class Token
//
// Defines a chunk of characters at a location in a Snek source file.
//
...
// ts.source_name();
//
// Gets the name of the Snek source file for this token stream.
//
...
// b = ts.at_number()
//
// Returns true if the next token's string is a decimal encoding of an int.
//
...
// b = ts.at_operator()
//
// Returns true if the next token is for an operator like **, <=, +=.
//
bool TokenStream::at_operator(void) {
    return is_operator(current().token);
}
...
// x = ts.eat_operator();
//
// Consumes the next token if it is an operator sequence like **, +=, >=.
// Raises an error if it isn't.
//
// Returns the operator as a string.
//
std::string TokenStream::eat_operator(void) {
    if (at_operator()) {
        std::string x = current().token;
        advance();
        return x;
    } else {
        std::string msg = "Syntax error: expected an operator ";
        msg += "but saw '";
        msg += current().token;
        msg += "' instead.";
        throw SnekError { locate(), msg };
 ...
 * is called, it repeatedly calls `advance_char` to process a Snek
...
// tz = Tokenizer {f};
//
// Constructs a Tokenizer when given the name of a .snk file.
// 
// Typical use of this module is:
//
//     Tokenizer   tz { "my_program.snk" };
    ...
    start_column { 1 }
{
    if (src_stream) {
        ...
        if (curr_char < 0) {
            throw SnekError { Locn { src_file_name, -1, -1 },
...
void Tokenizer::bail_with_error(std::string message) {
    throw SnekError { Locn {src_name, row, column}, message };
}
...
void Tokenizer::bail_with_char(std::string message) {
    throw SnekError { Locn {src_name, row, column },
                         message + "'" + curr_char + "'" };
}
...
// ts = tz.lex();
//    
// This runs the lexical analyzer on its Snek source.
    ...
    do {
        ...
        case INIT:
            if (curr_char >= '1' && curr_char <= '9') {
                ...
                // Is it a single-character delimiter?
                case '(':
                case ')':
                case ',':
                case ':':
        ...
        case OPER:
            //
            // Process the symbols of an operator.
            //
            switch (curr_char) {
            case '=':
            case '+':
...
// is_operator(s)
//
// Determines whether string `s` contains the symbols of an operator.
//
// Returns `true` or `false` accordingly.
//
